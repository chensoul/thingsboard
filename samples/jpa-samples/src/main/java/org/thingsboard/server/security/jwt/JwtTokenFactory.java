...
public class JwtTokenFactory {
	...
	public JwtToken createRefreshToken(SecurityUser securityUser) {
		...
		String token = setUpToken(securityUser, Collections.singletonList(Authority.REFRESH_TOKEN.name()), jwtSettingService.getJwtSetting().getRefreshTokenExpTime())
	...
	private JwtBuilder setUpToken(SecurityUser securityUser, List<String> scopes, long expirationTime) {
		...
		return Jwts.builder()
			.setClaims(claims)
			.setIssuer(jwtSettingService.getJwtSetting().getTokenIssuer())
			.setIssuedAt(Date.from(currentTime.toInstant()))
			.setExpiration(Date.from(currentTime.plusSeconds(expirationTime).toInstant()))
			.signWith(SignatureAlgorithm.HS512, jwtSettingService.getJwtSetting().getTokenSigningKey());
	}
