 ...
 */
package org.thingsboard.data.dao.aspect;
...
public class SqlDaoCallsAspect {
	...
	private final ConcurrentMap<String, DbCallStats> statsMap = new ConcurrentHashMap<>();
	...
	@Scheduled(initialDelayString = "${sql.log_tenant_stats_interval_ms:10000}",
		fixedDelayString = "${sql.log_tenant_stats_interval_ms:3000}")
	public void printStats() {
		...
		try {
			if (log.isTraceEnabled()) {
				...
				Map<String, Map<String, MethodCallStatsSnapshot>> byMethodStats = new HashMap<>();
	...
	@Around("@within(org.thingsboard.data.dao.aspect.SqlDao)")
	public Object handleSqlCall(ProceedingJoinPoint joinPoint) throws Throwable {
		...
		String tenantId = getTenantId(signature, methodName, joinPoint.getArgs());
		if (tenantId == null) {
	...
	private void reportSuccessfulMethodExecution(String tenantId, String method, long startTime) {
		reportMethodExecution(tenantId, method, true, startTime);
	}

	private void reportMethodExecution(String tenantId, String method, boolean success, long startTime) {
		statsMap.computeIfAbsent(tenantId, DbCallStats::new)
			.onMethodCall(method, success, System.currentTimeMillis() - startTime);
	}

	String getTenantId(MethodSignature signature, String methodName, Object[] args) {
		String tenantId = null;
		try {
			tenantId = SecurityUtils.getTenantId();
		} catch (ThingsboardException e) {
			log.trace("Failed to get tenantId from SecurityContext: {}", e.getMessage());
		}
