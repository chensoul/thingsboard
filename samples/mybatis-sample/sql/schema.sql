...
CREATE TABLE audit_log (
  `id` bigint NOT NULL AUTO_INCREMENT,
    ...
    entity_type varchar(255) CHECK (entity_type in ('TENANT', 'TENANT_PROFILE', 'MERCHANT', 'USER', 'ROLE',  'NOTIFICATION_TARGET', 'NOTIFICATION_TEMPLATE', 'NOTIFICATION_REQUEST', 'NOTIFICATION_RULE')),
...
-- Indices
CREATE UNIQUE INDEX uq_notification_rule_name ON notification_rule USING(tenant_id, name);
CREATE UNIQUE INDEX uq_notification_rule_external_id ON notification_rule USING (tenant_id, external_id);
CREATE INDEX idx_notification_rule_tenant_id_trigger_type_created_time ON notification_rule USING (tenant_id, trigger_type, created_time DESC);
