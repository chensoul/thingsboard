...
public class MailSender extends JavaMailSenderImpl {
	...
	private final MailContextComponent ctx;
	...
	public MailSender(MailContextComponent ctx, JsonNode jsonConfig) {
		super();
	...
	public void refreshAccessToken() {
		...
		try {
			if (System.currentTimeMillis() > getTokenExpires()) {
				SystemSetting settings = ctx.getSystemSettingService().findSystemSettingByType(SYS_TENANT_ID, SystemSettingType.MAIL);
				...
				if (MailOauth2Provider.OFFICE_365.name().equals(providerId)) {
					...
					((ObjectNode) jsonValue).put("refreshTokenExpires", Instant.now().plus(Duration.ofDays(RefreshTokenExpCheckService.AZURE_DEFAULT_REFRESH_TOKEN_LIFETIME_IN_DAYS)).toEpochMilli());
					ctx.getSystemSettingService().saveSystemSetting(SYS_TENANT_ID, settings);
				}
	...
	private int parsePort(String strPort) {
		try {
			...
			throw new DataValidationException(String.format("Invalid smtp port value: %s", strPort));
		}
	}
}
