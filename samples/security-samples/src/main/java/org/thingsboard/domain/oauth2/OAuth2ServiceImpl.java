...
public class OAuth2ServiceImpl implements OAuth2Service {
	...
	public OAuth2Info findOAuth2Info() {
		...
		oauth2ParamsList.stream().sorted(Comparator.comparing(BaseData::getId)).forEach(oauth2Params -> {
			List<OAuth2Registration> registrations = oauth2RegistrationDao.findByOAuth2ParamId(oauth2Params.getId());
			List<OAuth2Domain> domains = oauth2DomainDao.findByOAuth2ParamId(oauth2Params.getId());
			List<OAuth2Mobile> mobiles = oauth2MobileDao.findByOAuth2ParamId(oauth2Params.getId());
	...
	public List<OAuth2Registration> findAllRegistrations() {
		return oauth2RegistrationDao.findByTenantId(SYS_TENANT_ID);
	}
