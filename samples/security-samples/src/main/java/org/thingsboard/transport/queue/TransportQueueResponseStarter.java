package org.thingsboard.transport.queue;

import jakarta.annotation.PostConstruct;
import jakarta.annotation.PreDestroy;
import java.util.concurrent.ExecutorService;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.beans.factory.annotation.Value;
import org.springframework.boot.context.event.ApplicationReadyEvent;
import org.springframework.stereotype.Component;
import org.thingsboard.common.util.AfterStartUp;
import org.thingsboard.common.stats.MessagesStats;
import org.thingsboard.common.stats.StatsFactory;
import org.thingsboard.common.stats.StatsType;
import org.thingsboard.common.util.ThingsBoardExecutors;
import org.thingsboard.domain.message.ProtoQueueMsg;
import org.thingsboard.domain.message.TransportApiRequestMsg;
import org.thingsboard.domain.message.TransportApiResponseMsg;
import org.thingsboard.queue.common.DefaultQueueResponseTemplate;
import org.thingsboard.queue.provider.CoreQueueFactory;
import org.thingsboard.queue.spi.QueueConsumer;
import org.thingsboard.queue.spi.QueueProducer;
import org.thingsboard.queue.spi.QueueResponseTemplate;

/**
 * TODO Comment
 *
 * @author <a href="mailto:ichensoul@gmail.com">chensoul</a>
 * @since TODO
 */
@Slf4j
@RequiredArgsConstructor
@Component
public class TransportQueueResponseStarter {
	private final CoreQueueFactory coreQueueFactory;
	private final TransportQueueHandler transportQueueHandler;
	private final StatsFactory statsFactory;

	@Value("${queue.transport_api.max_pending_requests:10000}")
	private int maxPendingRequests;
	@Value("${queue.transport_api.max_requests_timeout:10000}")
	private long requestTimeout;
	@Value("${queue.transport_api.request_poll_interval:25}")
	private int responsePollDuration;
	@Value("${queue.transport_api.max_callback_threads:100}")
	private int maxCallbackThreads;

	private ExecutorService transportCallbackExecutor;

	private QueueResponseTemplate<ProtoQueueMsg<TransportApiRequestMsg>, ProtoQueueMsg<TransportApiResponseMsg>> transportQueueResponseTemplate;

	@PostConstruct
	public void init() {
		this.transportCallbackExecutor = ThingsBoardExecutors.newWorkStealingPool(maxCallbackThreads, getClass());
		QueueProducer<ProtoQueueMsg<TransportApiResponseMsg>> producer = coreQueueFactory.createTransportApiResponseProducer();
		QueueConsumer<ProtoQueueMsg<TransportApiRequestMsg>> consumer = coreQueueFactory.createTransportApiRequestConsumer();

		String key = StatsType.TRANSPORT.getName();
		MessagesStats queueStats = statsFactory.createMessagesStats(key);

		DefaultQueueResponseTemplate.DefaultQueueResponseTemplateBuilder
			<ProtoQueueMsg<TransportApiRequestMsg>, ProtoQueueMsg<TransportApiResponseMsg>> builder = DefaultQueueResponseTemplate.builder();
		builder.requestTemplate(consumer);
		builder.responseTemplate(producer);
		builder.maxPendingRequests(maxPendingRequests);
		builder.requestTimeout(requestTimeout);
		builder.pollInterval(responsePollDuration);
		builder.executor(transportCallbackExecutor);
		builder.stats(queueStats);
		transportQueueResponseTemplate = builder.build();
	}

	@AfterStartUp(order = AfterStartUp.REGULAR_SERVICE)
	public void onApplicationEvent(ApplicationReadyEvent applicationReadyEvent) {
		log.info("Received application ready event. Starting polling for events.");
		transportQueueResponseTemplate.init(transportQueueHandler);
	}

	@PreDestroy
	public void destroy() {
		if (transportQueueResponseTemplate != null) {
			transportQueueResponseTemplate.stop();
		}
		if (transportCallbackExecutor != null) {
			transportCallbackExecutor.shutdownNow();
		}
	}
}
